/*
 * Buzzer driver: GPIO 16 on/off with non-blocking beep sequencer.
 *
 * Two modes:
...
/* Altitude beep-out state */
static uint8_t alt_digits[6];   /* up to 6 digits */
...
void buzzer_set_altitude(int32_t altitude) {
    /* Convert to digits (e.g. 15280 → {1,5,2,8,0}) */
    if (altitude < 0) altitude = -altitude;
    if (altitude == 0) {
        alt_digits[0] = 0;
        alt_num_digits = 1;
    } else {
        /* Extract digits into temp array (reversed) */
        uint8_t tmp[6];
        int n = 0;
        int32_t v = altitude;
        while (v > 0 && n < 6) {
        ...
        /* Reverse into alt_digits */
        alt_num_digits = n;
        for (int i = 0; i < n; i++)
            alt_digits[i] = tmp[n - 1 - i];
    }
    alt_digit_idx = 0;
...
void buzzer_update(uint32_t now_ms) {
    ...
    case BZ_ALT_LONG_PAUSE:
        if (elapsed >= ALT_LONG_PAUSE_MS) {
            buzzer_tone_on();
            phase = BZ_ALT_LONG_BEEP;
            phase_start = now_ms;
        }
        break;

    case BZ_ALT_LONG_BEEP:
        if (elapsed >= ALT_LONG_BEEP_MS) {
            buzzer_tone_off();
            alt_digit_idx = 0;
            phase = BZ_ALT_SHORT_PAUSE;
            phase_start = now_ms;
        }
        break;

    case BZ_ALT_SHORT_PAUSE:
        if (elapsed >= ALT_SHORT_PAUSE_MS)
            start_alt_digit(now_ms);
        break;
    ...
    case BZ_ALT_BEEP_ON:
        if (elapsed >= ALT_BEEP_ON_MS) {
            buzzer_tone_off();
            alt_beeps_remaining--;
            if (alt_beeps_remaining > 0) {
                phase = BZ_ALT_BEEP_GAP;
            } else {
                alt_digit_idx++;
                if (alt_digit_idx < alt_num_digits) {
                    phase = BZ_ALT_DIGIT_GAP;
                } else {
                    /* All digits done — restart from long pause */
                    phase = BZ_ALT_LONG_PAUSE;
                }
            }
            phase_start = now_ms;
